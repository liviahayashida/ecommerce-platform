...
app = Flask(__name__) #instancia app do flask
...
def add_product():
    ...
    if 'name' in data and 'price' in data:
        product=Product(name=data["name"], price=data["price"], description=data.get("description", "Texto vazio"))
...
def add_to_cart(product_id):
    #usuario
    user = User.query.get(current_user.id) #instancia o user
    #produto
    product=Product.query.get(product_id)
...
def checkout():
    ...
    for cart_item in cart_items:
        db.session.delete(cart_item)
    db.session.commit()
    return jsonify({'message': 'Cchecout successful. Cart has been cleared'})
